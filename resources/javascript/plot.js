...
window.onload = function () {
    ...
    document.getElementById("addFunction").onmousedown = function (event) {
        if (plot.numberOfFunctions >= 5) { return; }
    ...
    document.getElementById("removeFunction").onmousedown = function (event) {
        if (plot.numberOfFunctions <= 1) { return; }
        ...
        plot.numberOfFunctions--;
        if (plot.numberOfFunctions < plot.functionParseTrees.length) { plot.functionParseTrees.pop(); }
        plot.drawEverything();
    };
    ...
    document.getElementById("plot").onmousewheel = function (event) {
        if (event.deltaY > 0) { plot.zoomOut(1.2); }
        if (event.deltaY < 0) { plot.zoomIn(1.2); }
    };
};